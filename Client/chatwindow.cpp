...
void ChatWindow::sendMessage()
{
    ...
    m_lastUserName.clear();
    //prevent user name appearin the list of online users
    //ui->listWidget->clear();
}
...
void ChatWindow::userJoined(const QString &username)
{
    ...
    m_chatModel->setData(m_chatModel->index(newRow, 0), tr("%1 Joined to the Chat").arg(username));
...
void ChatWindow::reciveOnlineUsers(const QJsonArray &users){
    ...
    {
        if(users.at(i).toString()==m_chatClient->currentNameUser)
            continue;
...
void ChatWindow::on_ButtonChatOneUser_clicked()
{
    if(ui->listWidget->selectedItems().count()>0){
        ...
        if(this->privateChats.contains(userNameSelected))
          return;
        this->privateChats.append(userNameSelected);
        chatOneUser oneChatDialog(this,userNameSelected,this->m_chatClient);
...
{
    if(!this->privateChats.contains(sender) ){
        this->privateChats.append(sender);
        chatOneUser oneChatDialog(this,sender,this->m_chatClient);
        oneChatDialog.setModal(true);
        emit signalSendMsgToDialog(sender,text);
        oneChatDialog.exec();
        this->privateChats.removeOne(sender);
        return;
    }
     emit signalSendMsgToDialog(sender,text);
}
